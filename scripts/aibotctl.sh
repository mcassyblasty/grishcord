...
SCRIPT_NAME="./scripts/aibotctl.sh"
DEFAULT_BOT_TTL_MS="900000"

log() { printf "[aibotctl] %s\n" "$*"; }
warn() { printf "[aibotctl][warn] %s\n" "$*" >&2; }
err() { printf "[aibotctl][error] %s\n" "$*" >&2; }

usage() {
  cat <<USAGE
Usage:
  $SCRIPT_NAME ollama <install|update|start|stop> [--bind-mode secure|docker]
  $SCRIPT_NAME bot <install|config|show|restart>
  $SCRIPT_NAME restart
  $SCRIPT_NAME help

Command groups:
  ollama   Manage Ollama runtime on the host (install/update/start/stop)
  bot      Manage bot identity/config and lifecycle helpers

Aliases (backward compatible):
  $SCRIPT_NAME install|update|start|stop [--bind-mode secure|docker]
USAGE
}

usage_bot() {
  cat <<USAGE
Usage: $SCRIPT_NAME bot <install|config|show|restart|help>
...
Bot commands:
  ...
  restart   Recreate only the bot container (no dependency recreation)
  help      Show this help
USAGE
}
...
run_root() {
  if [[ "${EUID}" -eq 0 ]]; then
    ...
    err "This action requires root. Re-run with sudo/root."
    exit 1
  fi
}
...
compose_cmd() {
  if [[ -f "$ENV_FILE" ]]; then
    docker compose --project-directory "$ROOT_DIR" --env-file "$ENV_FILE" -p "$PROJECT_NAME" -f "$COMPOSE_FILE" "$@"
  else
    docker compose --project-directory "$ROOT_DIR" -p "$PROJECT_NAME" -f "$COMPOSE_FILE" "$@"
  fi
}
...
bind_mode_to_host() {
  ...
  case "$mode" in
    ...
    *)
      err "Invalid bind mode '$mode'. Use 'secure' or 'docker'."
      exit 2
      ;;
  esac
}
...
save_env() {
  ...
  log "Saved config to $OLLAMA_ENV_FILE"
}

install_ollama_binary() {
  need_cmd curl
  log "Installing/updating Ollama via official installer..."
...
wait_for_ollama_ready() {
  ...
  local attempts=60
  local i
  log "Waiting for Ollama API readiness at $url ..."
  ...
  err "Timed out waiting for Ollama API readiness at $url"
  return 1
}
...
prompt_bind_mode() {
  ...
  case "$answer" in
    ...
    *)
      err "Invalid mode selection: $answer"
      exit 2
      ;;
  esac
}
...
cmd_install() {
  ...
  log "Done. Ollama is enabled and restarted with bind mode '$bind_mode'."
}
...
cmd_update() {
  ...
  if [[ "$keep" =~ ^[Nn]$ ]]; then
    warn "You chose to delete downloaded Ollama models from: $models_path"
    local confirm
    read -r -p "Type DELETE to confirm model deletion: " confirm
    if [[ "$confirm" != "DELETE" ]]; then
      err "Confirmation not received; aborting model deletion."
      exit 1
    fi
    run_root mkdir -p "$models_path"
...
cmd_restart() {
  ...
  log "Recreating bot container (no dependency recreation)..."
  compose_cmd rm -sf bot >/dev/null 2>&1 || true
  compose_cmd up -d --no-deps --force-recreate bot

  log "Bot container recreated."
  compose_cmd ps bot || true
  log "Recent bot logs:"
  compose_cmd logs --tail 50 bot || true
}
...
write_bot_config() {
  ...
  log "Saved bot config to $AIBOT_ENV_FILE"
}

interactive_bot_config() {
  load_env
  local ollama_model_from_env="${OLLAMA_MODEL:-}"
  ...
  local def_model="${OLLAMA_MODEL:-${ollama_model_from_env:-$DEFAULT_MODEL}}"
  local def_ttl="${BOT_CONVO_TTL_MS:-$DEFAULT_BOT_TTL_MS}"
...
cmd_bot_show() {
  if [[ ! -f "$AIBOT_ENV_FILE" ]]; then
    log "No $AIBOT_ENV_FILE found yet. Run install first."
    return 0
  fi
  log "Current bot config (non-secret):"
  awk -F= '!/^\s*#/ && NF>=2 { if ($1 != "BOT_PASSWORD") print $0 }' "$AIBOT_ENV_FILE"
}

parse_bind_mode_options() {
  BIND_MODE_OVERRIDE=""
  while [[ $# -gt 0 ]]; do
    case "$1" in
  ...
  if [[ -n "$BIND_MODE_OVERRIDE" ]]; then
    case "$BIND_MODE_OVERRIDE" in
      secure|docker) ;;
      *) err "Invalid --bind-mode value: $BIND_MODE_OVERRIDE"; exit 2 ;;
    esac
  fi
}
...
dispatch_ollama_command() {
  ...
  case "$cmd" in
    ...
    help|-h|--help) usage ;;
    *)
      err "Unknown ollama command: $cmd"
      usage
      exit 2
      ;;
  esac
...
dispatch_bot_command() {
  ...
  case "$bot_cmd" in
    ...
    *) err "Unknown bot command: $bot_cmd"; usage_bot; exit 2 ;;
  esac
}
...
main() {
  ...
  case "$cmd" in
    ollama)
      dispatch_ollama_command "${1:-help}" "${@:2}"
      ;;
    bot)
      dispatch_bot_command "${1:-help}"
      ;;
    restart)
      cmd_restart
      ;;
    install|update|start|stop)
      dispatch_ollama_command "$cmd" "$@"
      ;;
    help|-h|--help)
      usage
      ;;
