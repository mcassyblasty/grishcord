...
\subsection{Layouts}
Two responsive layouts are supported: (1) wide layout with left channel list, main chat panel, and right admin panel when applicable; (2) portrait layout with channel list in a drawer/hamburger pattern and full-screen chat timeline. Authentication screen is login-first, with registration and recovery panels hidden behind explicit menu toggles.
\subsection{Themes}
Two themes only: OLED ($\#000$ background, $\#fff$ text) and Discord-like dark palette. Theme switching is available from the in-app settings menu after login.
\subsection{Typography}
Inter is used for readability.

\section{Identity and Authentication}
Users have immutable \texttt{username} and mutable \texttt{display\_name}. Admin is the account with username \texttt{mcassyblasty}. Sessions are JWT cookies (HttpOnly) carrying user id and session version.
...
One server only. Admin can create, rename, reorder, archive channels. DMs are open to all users. Messages are plain text only; no markdown or HTML rendering. Storage limit is 10{,}000 chars; timeline initially renders first 2{,}000 chars with a Read More expansion to full stored text.
...
Admin panel includes invite generation, invite revocation, recovery link generation, user disable/enable controls, global anti-spam level, and voice bitrate controls. Anti-spam exact mapping:
\begin{center}
\begin{tabular}{rrrr}
\toprule
...
\section{Voice}
Voice uses open-source LiveKit SFU and client SDK, with exactly two lobbies: \texttt{lobby-a} and \texttt{lobby-b}. Each lobby enforces max 10 participants. UI exposes join/leave, mute/unmute, and participant list. TURN uses coturn and DNS-only hostname. Admin can configure publish audio max bitrate (default 32kbps, bounded).
