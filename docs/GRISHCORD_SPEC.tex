...
One server only. Admin can create, rename, reorder, and archive both text and voice channels through the Admin Settings channel editor. DMs are open to all users and exposed via a dedicated DMs navigation entry; the DM list is ordered by most recent conversation activity (newest to oldest) and refreshed in near-realtime for fast peer appearance. Messages are plain text only; no markdown or HTML rendering. Multiline message content is preserved end-to-end (including explicit newlines entered with Shift+Enter). Storage limit is 10{,}000 chars; timeline initially renders first 2{,}000 chars with a Read More expansion to full stored text. When no messages exist, the authenticated chat view shows an explicit empty-state prompt.
...
\begin{itemize}
  \item \textbf{0.3.13}: Added verified user-deletion flow in Admin Settings (exact username re-entry + confirmation checkbox), with safe server-side cleanup.
  \item \textbf{0.3.12}: Fixed runtime version display resolution and preserved Shift+Enter multiline message newlines in the timeline.
  \item \textbf{0.3.11}: Added Admin Mode and channel editor (add/rename/reorder/archive text+voice channels), plus snappier DM list refresh behavior.
  \item \textbf{0.3.10--0.3.1}: Sidebar/auth UX iterations, CSP-safe frontend script loading, and HTTP session-cookie reliability fixes.
\end{itemize}
