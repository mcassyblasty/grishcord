...
Admin panel includes one-click invite generation, invite revocation, recovery key generation, user disable/enable controls, verified user deletion (exact username re-entry + explicit checkbox), global anti-spam level, an explicit Admin Mode toggle that enables moderation/editor controls in the main app (including message deletion and channel editing). Anti-spam exact mapping:
\begin{center}
\begin{tabular}{rrrr}
\toprule
...
Docker Compose services: reverse proxy (Caddy), frontend, backend (API+WS), Postgres, and bot. Persistent bind mounts: \texttt{/mnt/grishcord/postgres}, \texttt{/mnt/grishcord/uploads}, and \texttt{/mnt/grishcord/config}.
...
\begin{itemize}
  \item v1.0.3: fixed LAN invalid-redirect regressions by redirecting only the configured public host to HTTPS while leaving local IP HTTP access functional.
  \item v1.0.2: added explicit HTTP→HTTPS Caddy split, HTTPS readiness checks in grishcordctl, and Cloudflare 522 troubleshooting guidance.
  \item v1.0.1: enabled automatic HTTPS in Caddy with hostname-based config, plus env/preflight guardrails for reliable update-start redeploys.
  \item v1.0.0: added announcement-only channel mode (admin-only posting, everyone can read) with admin toggles and UI indicators.
  \item v0.3.50: notifications menu now supports browser notification permission + toggle, and realtime app notifications mirror to browser notifications when allowed.
  \item v0.3.49: non-image attachment labels now use display-order numbering and toast errors are shown top-center for visibility.
  \item v0.3.48: edit attachment chips are numbered by editor display order (1..N) instead of upload DB IDs.
  \item v0.3.47: attachment-only message edits now save correctly, including removing/replacing files on messages with empty text bodies.
  \item v0.3.46: edit attachment chips now use user-facing labels instead of raw upload IDs; sequence numbers appear only when multiple attachments are present.
  \item v0.3.45: message edit flow now supports adding/removing allowed attachments (images + .zip) on existing messages.
