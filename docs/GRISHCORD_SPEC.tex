...
Users have immutable \texttt{username} and mutable \texttt{display\_name}. Admin is the account with username \texttt{mcassyblasty}. Sessions are JWT cookies (HttpOnly) carrying user id and session version. Logout invalidates the server-side session version immediately and clears the auth cookie with matching attributes. Secure-cookie mode is applied only on HTTPS requests so LAN HTTP testing can still authenticate correctly while preserving secure behavior under TLS.
...
Admin panel includes one-click invite generation, invite revocation, recovery key generation, user disable/enable controls, global anti-spam level, and voice bitrate controls. Anti-spam exact mapping:
\begin{center}
\begin{tabular}{rrrr}
\toprule
...
\begin{itemize}
  \item \textbf{0.3.10}: Immediate logout session invalidation (server-side token version bump) and reliable cookie clearing semantics.
  \item \textbf{0.3.9}: Added working browser voice transmission (WebRTC signaling over WS), admin message deletion, multiline compose with Shift+Enter, and one-click invite generation UX.
  \item \textbf{0.3.8}: DMs switch ordering preference and key-first admin invite/recovery outputs.
  \item \textbf{0.3.7--0.3.1}: Sidebar/auth UX iterations, CSP-safe frontend script loading, and HTTP session-cookie reliability fixes.
\end{itemize}
